...
void map::findPath(int originX, int originY, int destX, int destY) {
	...
	for (int i = 0; i < route.length; i++) {
		changeSymbol(NodeAccessor(route.location[i])->location%width,
					 NodeAccessor(route.location[i])->location/height,
					 'T' //Default traceur symbol - Feel free to change
					 );
		if (i > 0) {
			changeSymbol(NodeAccessor(route.location[i - 1])->location%width,
						 NodeAccessor(route.location[i - 1])->location/height,
