...
path graph::ShortestPath(int start,int end){
	...
	path returnPath(size);
	returnPath.location[0] = start;//Don't forget to include the first node!
	int position = 0;//keeps track of our traversal through final path
	while(position < pathlen){
		current = backtrack[pathlen - position];//the end of backtrack is the beginning of path we want
		position++;
		returnPath.location[position] = current;
	}
...
int main(){
	...
	path route(map.ShortestPath(2,5));
